...
bool Population::epoch(int generation) {
	...
	*/
	...
	//첫Organism의 pop champ to true
	//fitness가 큰 순서대로 정렬했으므로 첫번째가 기존보다 크면 변경, 아니라면 ..
	...
	if (highest_last_changed>=NEAT::dropoff_age+5) {
		...
		(*(((*curspecies)->organisms).begin()))->super_champ_offspring=half_pop; //첫번째 organism의 super_champ..를 population의 절반으로 설정
		(*curspecies)->expected_offspring=half_pop; // species의 생성될 자식수를 population의 절반으로 설정
