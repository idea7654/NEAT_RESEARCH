...
Genome::Genome(int id, std::vector<Trait*> t, std::vector<NNode*> n, std::vector<Link*> links) {
	...
	for (auto &i : links) {
		...
		tempgene = new Gene(i->linktrait, i->weight, i->in_node, i->out_node, i->is_recurrent, 1.0, 0.0);
...
void Genome::mutate_random_trait() {
	...
	traitnum = randbtn(0, traits.size() - 1);
...
void Genome::mutate_link_trait(int times) {
	...
	for (loop = 1; loop <= times; loop++) {
		...
		genenum = randbtn(0, genes.size() - 1);
...
void Genome::mutate_node_trait(int times) {
	...
	for (loop = 1; loop <= times; loop++) {
		...
		nodenum = randbtn(0, nodes.size() - 1);
...
bool Genome::mutate_add_link(std::vector<Innovation*> &innovs, double &curinnov, int tries) {
	...
	if (randbtn(0.0, 1.0) < NEAT::recur_only_prob)
		do_recur = true;
	else do_recur = false;
	...
	if (do_recur) {
		...
		while (trycount < tries) {
			...
			if (loop_recur) {
				...
				nodenum1 = randbtn(first_nonsensor, nodes.size() - 1);
				nodenum2 = nodenum1;
			}
			else {
				...
				nodenum1 = randbtn(0, nodes.size() - 1);
				nodenum2 = randbtn(first_nonsensor, nodes.size() - 1);
			}
...
void Genome::mutate_add_sensor(std::vector<Innovation*> &innovs, double &curinnov) {
	...
	sensor = sensors[randbtn(0, sensors.size() - 1)];
		...
		if (p1gene == genes.end()) {
			...
			if (p1innov == p2innov) {
				...
				if ((((*p1gene)->enable) == false) ||
					...
					if (randbtn(0.0, 1.0) < 0.75) disable = true;
	...
	while (!((p1gene == genes.end()) &&
		...
		if (p1gene == genes.end()) {
			...
			if (p1innov == p2innov) {
				...
				if(randbtn(0.0, 1.0) > 0.5) (avgene->lnk)->linktrait = ((*p1gene)->lnk)->linktrait;
				...
				if (randbtn(0.0, 1.0) > 0.5) (avgene->lnk)->out_node = ((*p1gene)->lnk)->out_node;
				else (avgene->lnk)->out_node = ((*p2gene)->lnk)->out_node;

				if (randbtn(0.0, 1.0) > 0.5) (avgene->lnk)->is_recurrent = ((*p1gene)->lnk)->is_recurrent;
				...
				if ((((*p1gene)->enable) == false) ||
					(((*p2gene)->enable) == false))
					if (randbtn(0.0, 1.0) < 0.75) avgene->enable = false;
...
Genome *Genome::mate_singlepoint(Genome *g, int genomeid) {
	...
	if (genes.size() < (g->genes).size()) {
		...
		crosspoint = randbtn(0, genes.size() - 1);
		p1gene = genes.begin();
		p2gene = (g->genes).begin();
		stopper = (g->genes).end();
		p1stop = genes.end();
		...
		crosspoint = randbtn(0, (g->genes).size() - 1);
		p2gene = genes.begin();
