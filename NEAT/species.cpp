...
bool Species::reproduce(int generation, Population *pop, std::vector<Species*> &sorted_species) {
	...
	for (count = 0; count < expected_offspring; count++) {
		...
		//If there is only one organism in the pool, then always mutate
		else if (randbtn(0.0, 1.0) < NEAT::mutate_only_prob || poolsize == 0) {

			//Choose the random parent

			//RANDOM PARENT CHOOSER
			//orgnum = randint(0, poolsize);
			orgnum = randbtn(0, poolsize);
			curorg = organisms.begin();
			...
			//       will not be appropriately altered to reflect the change
				...
				if (randbtn(0.0, 1.0) < NEAT::mutate_random_trait_prob) {
					//std::cout<<"mutate random trait"<<std::endl;
					new_genome->mutate_random_trait();
				}
				if (randbtn(0.0, 1.0) < NEAT::mutate_link_trait_prob) {
					//std::cout<<"mutate_link_trait"<<std::endl;
					new_genome->mutate_link_trait(1);
				}
				if (randbtn(0.0, 1.0) < NEAT::mutate_node_trait_prob) {
				...
				if (randbtn(0.0, 1.0) < NEAT::mutate_gene_reenable_prob) {
		...
		//Otherwise we should mate 
			...
			//Choose random dad
			if (randbtn(0.0, 1.0) > NEAT::interspecies_mate_rate) {
				//Mate within Species
			...
			if (randbtn(0.0, 1.0) < NEAT::mate_multipoint_prob) {
				new_genome = (mom->gnome)->mate_multipoint(dad->gnome, count, mom->orig_fitness, dad->orig_fitness, outside);
			}
			else if (randbtn(0.0, 1.0) < (NEAT::mate_multipoint_avg_prob / (NEAT::mate_multipoint_avg_prob + NEAT::mate_singlepoint_prob))) {
			...
			if ((randbtn(0.0, 1.0) > NEAT::mate_only_prob) ||
